...
module.exports = {
  ...
  getNature: ({ attribute, attributeName, modelName }) => {
    ...
    const models = siapi.db.getModelsByPluginName(attribute.plugin);

    const pluginModels = Object.values(siapi.plugins).reduce((acc, plugin) => {
      return acc.concat(Object.values(plugin.models));
    }, []);

    const allModels = Object.values(siapi.models).concat(pluginModels);
  ...
  defineAssociations: function(model, definition, association, key) {
    try {
      ...
      const targetModel =
        targetName !== '*' ? siapi.db.getModel(targetName, association.plugin) : null;
      ...
      if (targetName !== '*') {
        const model = siapi.db.getModel(targetName, association.plugin);
      ...
      const pluginsModels = Object.keys(siapi.plugins).reduce((acc, current) => {
        Object.keys(siapi.plugins[current].models).forEach(entity => {
          Object.keys(siapi.plugins[current].models[entity].attributes).forEach(attribute => {
            const attr = siapi.plugins[current].models[entity].attributes[attribute];

            if ((attr.collection || attr.model || '').toLowerCase() === model.toLowerCase()) {
              acc.push(siapi.plugins[current].models[entity]);
            }
          });
        });
      ...
      const appModels = Object.keys(siapi.models).reduce((acc, entity) => {
        Object.keys(siapi.models[entity].attributes).forEach(attribute => {
          const attr = siapi.models[entity].attributes[attribute];

          if ((attr.collection || attr.model || '').toLowerCase() === model.toLowerCase()) {
            acc.push(siapi.models[entity]);
          }
        });
      ...
      const componentModels = Object.keys(siapi.components).reduce((acc, entity) => {
        Object.keys(siapi.components[entity].attributes).forEach(attribute => {
          const attr = siapi.components[entity].attributes[attribute];

          if ((attr.collection || attr.model || '').toLowerCase() === model.toLowerCase()) {
            acc.push(siapi.components[entity]);
          }
        });
