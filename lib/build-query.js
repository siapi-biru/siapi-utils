 ...
 * @param {Object} options.model - Siapi model
  ...
  return whereClauses
    .filter(({ field, value }) => {
      if (_.isNull(value)) {
        ...
        siapi.log.warn(`The value of field: '${field}', in your where filter, is undefined.`);
        return false;
      }
      return true;
    })
...
const buildQuery = ({ model, filters = {}, ...rest }) => {
  ...
  return siapi.db.connectors.get(model.orm).buildQuery({ model, filters, ...rest });
};
